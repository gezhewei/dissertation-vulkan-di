...
App::App(std::string scene, bool ignorePointLights, unsigned lightSeed, std::optional<CameraPreset> cameraPreset, BenchmarkConfig benchmark, ExperimentConfig experiment) :
	_window({ { GLFW_CLIENT_API, GLFW_NO_API } },
		experiment.windowSize ? static_cast<int>(experiment.windowSize->width) : 800,
		experiment.windowSize ? static_cast<int>(experiment.windowSize->height) : 600),
	_benchmark(std::move(benchmark)),
	_experiment(std::move(experiment)),
	_lightSeed(lightSeed)
{
	VULKAN_HPP_DEFAULT_DISPATCHER.init(vkGetInstanceProcAddr);

	if (scene.empty()) {
		throw std::invalid_argument("A glTF scene must be specified with --scene=<path>");
	}

	// Apply command-line experiment configuration to the runtime parameters.
	_enableTemporalReuse = _experiment.temporalReuse;
	_temporalReuseSampleMultiplier = _experiment.temporalClamp;
	_spatialReuseIterations = _experiment.spatialIterations;
	_log2InitialLightSamples = _experiment.initialSamplesLog2;
	_visibilityTestMethod = _experiment.visibility;

	if (cameraPreset) {
		_camera.position = cameraPreset->position;
		_camera.lookAt = cameraPreset->lookAt;
		_lockCamera = true;
	}
	...
	_window.setKeyHandler([this](int key, int scancode, int action, int mods) {
		_onKeyEvent(key, scancode, action, mods);
		});
	...
	{ // pick physical device
		...
		for (const vk::PhysicalDevice& dev : physicalDevices) {
			...
			++discreteIndex;
			if (_experiment.deviceIndex >= 0 && discreteIndex != _experiment.deviceIndex) {
				continue;
			}
			bool supportsExtensions = checkSupport<&vk::ExtensionProperties::extensionName>(
			...
			_physicalDevice = dev;
			if (_experiment.deviceIndex >= 0) {
				break; // explicit device requested, stop after selection
			}
		}
	}
	if (!_physicalDevice) {
	...
	{ // create descriptor pools
		...
		std::array<vk::DescriptorPoolSize, 1> texturePoolSizes{
			vk::DescriptorPoolSize(vk::DescriptorType::eCombinedImageSampler, static_cast<uint32_t>(4 * _gltfScene.m_materials.size()))
		};
	...
	if constexpr (_unbiasedSpatialReuse) {
...
void App::updateGui() {
	...
	if (_benchmark.enabled) {
		// Benchmark mode renders no UI so that screenshots and frame times are clean.
		ImGui::Render();
		return;
	}
	...
	ImGui::TextUnformatted("Spatial Reuse: Biased (paper configuration)");
...
void App::mainLoop() {
	...
	std::chrono::steady_clock::time_point lastPresentTick = std::chrono::steady_clock::now();
	bool presentTickValid = false;

	while (!_window.shouldClose() || _screenshotReadbackPending) {
		glfwPollEvents();

		if (_screenshotReadbackPending) {
			_graphicsComputeQueue.waitIdle();
			_saveStagedScreenshot();
		...
		{ // record present command buffer
			...
			if (_wantScreenshotNextFrame) {
				_ensureScreenshotStaging();
				vk::Image swapImage = _swapchain.getImages()[imageIndex];
				vk::Extent2D captureExtent = _swapchain.getImageExtent();

				// Image is in ColorAttachmentOptimal layout after imgui pass
				...
				commandBuffer.pipelineBarrier(
					...
					vk::DependencyFlagBits(0), {}, {}, toTransferBarrier
				);

				vk::BufferImageCopy region;
				region.setImageExtent(vk::Extent3D(captureExtent.width, captureExtent.height, 1));
				region.setImageSubresource(vk::ImageSubresourceLayers(vk::ImageAspectFlagBits::eColor, 0, 0, 1));
				commandBuffer.copyImageToBuffer(
					swapImage, vk::ImageLayout::eTransferSrcOptimal, _screenshotStagingBuffer.get(), region
				);

				// Transition back to PresentSrcKHR for presentation
				...
				commandBuffer.pipelineBarrier(
					...
					vk::DependencyFlagBits(0), {}, {}, toPresentBarrier
				);

				_screenshotExtent = captureExtent;
				_wantScreenshotNextFrame = false;
		...
		{
			auto presentTick = std::chrono::steady_clock::now();
			float frameTime = presentTickValid ? std::chrono::duration<float>(presentTick - lastPresentTick).count() : 0.0f;
			lastPresentTick = presentTick;
			presentTickValid = true;
			_benchmarkAfterFrame(frameTime);
		}
...
void App::_onKeyEvent(int key, int scancode, int action, int mods) {
	ImGui_ImplGlfw_KeyCallback(_window.getRawHandle(), key, scancode, action, mods);
	if (action != GLFW_PRESS) {
		return;
	}
	if (key == GLFW_KEY_F9) {
		_interactiveScreenshotCounter += 1;
		std::filesystem::path path = "screenshot_" + std::to_string(_interactiveScreenshotCounter) + ".png";
		std::cout << "Screenshot requested -> " << path.string() << "\n";
		_nextScreenshotPath = path;
		_wantScreenshotNextFrame = true;
	}
}

void App::_benchmarkBeforeFrame() {
	if (!_benchmark.enabled || _benchmarkFinished) {
		return;
	}
	_benchmarkFrame += 1;
	if (_benchmarkFrame > _benchmark.warmupFrames) {
		_benchmarkRecording = true;
	}
	if (_benchmark.screenshotFrame) {
		uint64_t measuredFrame = _benchmarkFrame > _benchmark.warmupFrames ?
			_benchmarkFrame - _benchmark.warmupFrames : 0;
		if (_benchmarkRecording && measuredFrame == _benchmark.screenshotFrame.value() && !_wantScreenshotNextFrame && !_screenshotReadbackPending) {
			_nextScreenshotPath = _benchmark.screenshotPath.empty() ?
				std::filesystem::path("screenshot.png") : _benchmark.screenshotPath;
			_wantScreenshotNextFrame = true;
			std::cout << "Benchmark screenshot scheduled at measured frame " << measuredFrame << "\n";
		}
	}
}

void App::_benchmarkAfterFrame(float frameTimeSeconds) {
	if (!_benchmark.enabled || _benchmarkFinished) {
		return;
	}
	if (_benchmarkRecording && frameTimeSeconds > 0.0f) {
		_benchmarkFrameTimes.emplace_back(frameTimeSeconds);
		if (_benchmarkFrameTimes.size() % 100 == 0) {
			std::cout << "Benchmark measured " << _benchmarkFrameTimes.size() << "/" << _benchmark.measureFrames << " frames\n";
		}
	}
	if (_benchmarkRecording && _benchmarkFrameTimes.size() >= _benchmark.measureFrames) {
		_writeBenchmarkCsv();
		_benchmarkFinished = true;
		std::cout << "Benchmark finished. Frame times written to " << _benchmark.frameTimesCsv.string() << "\n";
		_window.requestClose();
	}
}

void App::_writeBenchmarkCsv() {
	std::ofstream out(_benchmark.frameTimesCsv, std::ios::out | std::ios::trunc);
	if (!out) {
		std::cerr << "Failed to open " << _benchmark.frameTimesCsv.string() << " for writing\n";
		return;
	}
	double sum = 0.0;
	for (float t : _benchmarkFrameTimes) {
		sum += t;
	}
	double meanSeconds = sum / static_cast<double>(_benchmarkFrameTimes.size());
	out << "frame,time_ms\n";
	for (std::size_t i = 0; i < _benchmarkFrameTimes.size(); ++i) {
		out << i << "," << std::fixed << std::setprecision(4) << (_benchmarkFrameTimes[i] * 1000.0) << "\n";
	}
	out << "mean," << std::fixed << std::setprecision(4) << (meanSeconds * 1000.0) << "\n";
	std::cout << "Mean frame time: " << std::fixed << std::setprecision(3) << (meanSeconds * 1000.0)
		<< " ms (" << std::setprecision(2) << (1.0 / meanSeconds) << " FPS) over "
		<< _benchmarkFrameTimes.size() << " frames\n";
}

void App::_ensureScreenshotStaging() {
	vk::Extent2D extent = _swapchain.getImageExtent();
	// 4 bytes per pixel (BGRA8), rows tightly packed
	vk::DeviceSize requiredSize = static_cast<vk::DeviceSize>(extent.width) * extent.height * 4;
	if (_screenshotStagingBuffer.get() && _screenshotBufferSize >= requiredSize) {
		return;
	}
	_screenshotBufferSize = requiredSize;
	_screenshotStagingBuffer = _allocator.createBuffer(
...
void App::_saveStagedScreenshot() {
	if (!_screenshotStagingBuffer.get()) {
		return;
	}
	void* mapped = _screenshotStagingBuffer.map();
	_screenshotStagingBuffer.invalidate();

	std::size_t width = _screenshotExtent.width;
	std::size_t height = _screenshotExtent.height;
	
	// Debug: print swapchain format and first few pixels
	std::cout << "Screenshot format: " << vk::to_string(_swapchain.getImageFormat()) << "\n";
	const auto* src = static_cast<const unsigned char*>(mapped);
	std::cout << "First 4 pixels (raw): ";
	for (int i = 0; i < 4; ++i) {
		std::cout << "(" << (int)src[i*4] << "," << (int)src[i*4+1] << "," << (int)src[i*4+2] << "," << (int)src[i*4+3] << ") ";
	}
	std::cout << "\n";
	
	std::vector<unsigned char> rgba(width * height * 4);
	for (std::size_t i = 0; i < width * height; ++i) {
		// Swapchain is sRGB format, but copyImageToBuffer reads linear values.
		// Apply sRGB gamma correction manually.
		auto toSrgb = [](unsigned char linear) -> unsigned char {
			float l = linear / 255.0f;
			float s = l <= 0.0031308f ? 12.92f * l : 1.055f * std::pow(l, 1.0f / 2.4f) - 0.055f;
			return static_cast<unsigned char>(std::clamp(s, 0.0f, 1.0f) * 255.0f + 0.5f);
		};
	...
	_screenshotStagingBuffer.unmap();

	std::filesystem::path target = _nextScreenshotPath.empty() ? std::filesystem::path("screenshot.png") : _nextScreenshotPath;
	int writeResult = stbi_write_png(
		target.string().c_str(),
		static_cast<int>(width), static_cast<int>(height), 4, rgba.data(),
		static_cast<int>(width * 4)
	);
	if (writeResult) {
		std::cout << "Screenshot saved: " << target.string() << " (" << width << "x" << height << ")\n";
	} else {
		std::cerr << "Failed to write screenshot to " << target.string() << "\n";
	}
	_nextScreenshotPath.clear();
}
