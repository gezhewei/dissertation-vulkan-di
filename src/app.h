...
// Command-line controlled benchmark configuration used by the dissertation evaluation.
struct BenchmarkConfig {
	bool enabled = false;
	uint64_t warmupFrames = 300;
	uint64_t measureFrames = 1000;
	std::optional<uint64_t> screenshotFrame; // measured-frame index at which to capture
	std::filesystem::path frameTimesCsv = "frame_times.csv";
...
// Command-line controlled experiment configuration used by the dissertation evaluation.
struct ExperimentConfig {
	bool temporalReuse = true;
	int temporalClamp = 20;
	int spatialIterations = 1;
	int initialSamplesLog2 = 5;
	VisibilityTestMethod visibility = VisibilityTestMethod::hardware;
	int deviceIndex = -1; // -1 = first suitable discrete GPU
	std::optional<vk::Extent2D> windowSize;
};
...
public:
	...
	App(std::string scene, bool ignorePointLights, unsigned lightSeed, std::optional<CameraPreset> cameraPreset, BenchmarkConfig benchmark, ExperimentConfig experiment);
	~App();
...
protected:
	...
	// Paper configuration: keep the visually preferred biased spatial reuse path.
	// Change this constant only when explicitly running an ablation experiment.
	inline static constexpr bool _unbiasedSpatialReuse = false;

	nvmath::vec2f _lastMouse;
	int _pressedMouseButton = -1;
	bool _cameraUpdated = true;

	// Benchmark / evaluation support
	BenchmarkConfig _benchmark;
	ExperimentConfig _experiment;
	unsigned _lightSeed = 0;
	bool _lockCamera = false;
	uint64_t _benchmarkFrame = 0; // frames presented since benchmark start (warmup included)
	bool _benchmarkRecording = false;
	bool _benchmarkFinished = false;
	std::vector<float> _benchmarkFrameTimes;

	vma::UniqueBuffer _screenshotStagingBuffer;
	vk::DeviceSize _screenshotBufferSize = 0;
	vk::Extent2D _screenshotExtent{ 0, 0 };
	bool _wantScreenshotNextFrame = false;
	bool _screenshotReadbackPending = false;
	std::filesystem::path _nextScreenshotPath;
	int _interactiveScreenshotCounter = 0;
	...
	void _onKeyEvent(int key, int scancode, int action, int mods);
	void _ensureScreenshotStaging();
	void _saveStagedScreenshot();
	void _writeBenchmarkCsv();
	void _benchmarkBeforeFrame();
	void _benchmarkAfterFrame(float frameTimeSeconds);
	...
	void _transitionGBufferLayouts() {
		...
		for (std::size_t i = 1; i < numGBuffers; ++i) {
			...
			transitionImageLayout(
				cmdBuf.get(), _gBuffers[i].getEmissionBuffer(), GBuffer::Formats::get().emission,
				vk::ImageLayout::eUndefined, vk::ImageLayout::eShaderReadOnlyOptimal
			);
			transitionImageLayout(
