...
namespace glfw {
	void _glfwCheck(bool returnValue) {
		if (returnValue != GLFW_TRUE) {
			...
			glfwGetError(&error);
			std::cout << "GLFW error: " << (error ? error : "unknown error") << "\n";
		#else
			std::cout << "GLFW error (GLFW 3.2 does not expose glfwGetError)\n";
		#endif
			std::abort();
		}
	}
	...
	Window::Window(std::initializer_list<std::pair<int, int>> hints, int width, int height) {
		_maybeInitGlfw();
