...
void GBuffer::resize(vma::Allocator &allocator, vk::Device device, vk::Extent2D extent, Pass &pass) {
	...
	_emissionBuffer = allocator.createImage2D(
		extent, formats.emission,
		vk::ImageUsageFlagBits::eColorAttachment | vk::ImageUsageFlagBits::eSampled
	);
	...
	_emissionView = createImageView2D(
		device, _emissionBuffer.get(), formats.emission, vk::ImageAspectFlagBits::eColor
	);
	...
	std::array<vk::ImageView, 6> attachments{
		_albedoView.get(), _normalView.get(), _materialPropertiesView.get(), _worldPosView.get(),
		_emissionView.get(), _depthView.get()
	};
...
void GBuffer::Formats::initialize(vk::PhysicalDevice physicalDevice) {
	...
	_gBufferFormats.emission = findSupportedFormat(
...
vk::UniqueRenderPass GBufferPass::_createPass(vk::Device device) {
	...
	std::array<vk::AttachmentReference, 5> colorAttachmentReferences{
