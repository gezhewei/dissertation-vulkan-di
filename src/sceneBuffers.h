...
public:
	...
	[[nodiscard]] static SceneBuffers create(
		...
		if (pointLights.empty() && triangleLights.empty()) {
			pointLights = generateRandomPointLights(
				200, scene.m_dimensions.min, scene.m_dimensions.max,
		...
		const vk::Format linearTextureFormat = vk::Format::eR8G8B8A8Unorm;
		const vk::Format srgbTextureFormat = vk::Format::eR8G8B8A8Srgb;

		std::vector<bool> srgbTextures(scene.m_textures.size(), false);
		auto markSrgbTexture = [&srgbTextures](int textureIndex) {
			if (textureIndex >= 0 && static_cast<std::size_t>(textureIndex) < srgbTextures.size()) {
				srgbTextures[textureIndex] = true;
			}
		};
		for (const nvh::GltfMaterial &material : scene.m_materials) {
			markSrgbTexture(material.pbrBaseColorTexture);
			markSrgbTexture(material.khrDiffuseTexture);
			markSrgbTexture(material.emissiveTexture);
		}
		...
		for (int i = 0; i < scene.m_textures.size(); ++i) {
			...
			const vk::Format textureFormat = srgbTextures[i] ? srgbTextureFormat : linearTextureFormat;
			...
			result._textureImages[i].image = loadTexture(
				gltfimage, textureFormat, numMipLevels, allocator, oneTimeBufferPool, graphicsQueue
			);
			...
			result._textureImages[i].imageView = createImageView2D(
				...
				textureFormat, vk::ImageAspectFlagBits::eColor, 0, numMipLevels
			);
		}
		...
		// generate default textures
		{
			...
			result._defaultNormal.imageView = createImageView2D(
				l_device, result._defaultNormal.image.get(), linearTextureFormat, vk::ImageAspectFlagBits::eColor
			);
			...
			result._defaultWhite.imageView = createImageView2D(
				l_device, result._defaultWhite.image.get(), linearTextureFormat, vk::ImageAspectFlagBits::eColor
			);
		}
