...
layout (binding = 4) uniform sampler2D uniEmission;
...
layout (binding = 7) buffer PointLights {
	int count;
	pointLight lights[];
} pointLights;
layout (binding = 8) buffer TriangleLights {
	int count;
	triLight lights[];
} triangleLights;
...
void main() {
	...
	vec3 surfaceEmission = texture(uniEmission, inUv).rgb;
